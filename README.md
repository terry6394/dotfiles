...
- **Machine label** `"desk"` in `flake.nix` (the `machine = "desk"` line): selects the per-machine software tier in `configuration.nix`. Common lists install on every machine; `machineBrews`/`machineCasks`/`machineTaps` only install on the named machine. Add a machine by extending those attrsets and setting `machine` in `flake.nix` on that machine. `bootstrap.sh` detects the host name and asks to confirm or change the label on first setup; `rebuild.sh` refuses to run without a label.
